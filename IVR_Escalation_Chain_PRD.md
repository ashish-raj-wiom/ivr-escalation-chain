...
| | | | |
|---|---|---|---|
| **Owner** — Ashish Raj (PM) | **Reviewer** — TBD (Eng Lead) ⚠️ *AI GENERATED — review* | **Status** — Draft | **Sign-off** — Pending |
| **Version** — v0.1 · 3 Aug 2026 | **Consulted — CSP Ops** — TBD ⚠️ *AI GENERATED — review* | **Consulted — Telephony (Exotel)** — TBD ⚠️ *AI GENERATED — review* | **Consulted — Partner Platform** — TBD ⚠️ *AI GENERATED — review* |

---

## 1. Objective & Definition of Success

**Objective.** A customer who calls about their restore or pickup ticket reaches a person at their own CSP — if the assigned technician does not answer, the call moves on by itself to his manager and then to the owner, and the customer does nothing but wait.

**Boundary.** This spec governs **customer-initiated** IVR calls on Service (restore) and Pickup tickets for CSPs in the Phase-1 cohort (C-01, C-03). It leaves unchanged:

- **CSP-initiated calls** to customers — a separate PRD covers reaching the customer on a second number (AC-REG-1).
- **Install tickets** — out of scope; install already connects at a higher rate and is the source of this spec's target (AC-REG-4). ⚠️ *AI GENERATED — review*
- **Calls on a ticket with no technician assigned** — today's behaviour stands, untouched (T6, AC-REG-2).
...
- **What the answering person is told about the call** — no ticket context, customer name or escalation indicator is delivered to any rung. Out of scope (AC-REG-5).
...
| G2 | **The technician is always first** | Every chain starts at the assigned technician, so escalation never becomes the normal route into a CSP. | R2 · AC-GRD-2 · MQ-3 · MQ-7 |
...
Ticket-level connect rate is defined in §8. A ticket where the customer hung up before any rung answered **counts as not connected** (T5, MQ-5).

| ID | Metric | Baseline | Target | Source |
|---|---|---|---|---|
| M1 | Ticket-level connect rate — customer-initiated, **Service** | 50.2% *(2 Jul – 3 Aug 2026, Phase-1 cohort)* | 69.0% | MQ-1 |
| M2 | Ticket-level connect rate — customer-initiated, **Pickup** | 51.1% *(same window)* | 69.0% | MQ-1 |

**Where the target comes from.** 69.0% is what install tickets already achieve on customer-initiated calls over the same window and cohort — the closest thing to a ceiling this direction is known to reach. It is direction-matched on purpose: install's all-directions figure (79.6%) mixes in CSP-initiated calls, and its CSP-initiated figure (75.8%) belongs to the customer-side PRD, so neither can be used here.
| M3 | Share of in-scope tickets that connect at **rung 1** (the technician) | 50.2% *(Service — today every connect is the technician)* | ≥ 50.2% — must not fall below baseline | MQ-3 |

M3 is the counter-metric for M1. M1 rising while M3 falls means technicians have stopped answering and the owner is absorbing the calls — connect rate would improve while service got worse, because an owner answering does not restore anyone's line.

**Do not derive M1's target from independence.** Three rungs each answering at ~50% gives 87.5% only if the three failures are unrelated. If a common cause suppresses all three, the chain moves M1 very little. MQ-3 measures per-rung answer rate so this is known within a month of launch rather than assumed.

**Invariant (not a metric):** G2 — chains where an assigned technician was not rung 1 = 0, zero tolerance. Monitored via MQ-7, not trended.
...
| R2 | As a CSP owner, I want my technician to get the call first, so escalation stays the exception. | Dial the assigned technician as rung 1 on every chain (G2). | Skip, reorder or bypass the technician when one is assigned and has a number. |
| R3 | As a CSP owner who is also my own technician, I do not want my phone rung three times for one call. | Dial each distinct person once; where roles resolve to the same person, shorten the chain (G3). | Dial the same number more than once in one chain. |
...
| R5 | As a technician who missed a call, I want the next call on that ticket to reach me first. | Treat every inbound call as a new chain that starts at the technician. | Resume a later call part-way down the chain, or remember which rungs already failed. |
...
|---|---|---|---|---|---|
| T1 | — | Customer-initiated call bridged, caller resolved to an in-scope ticket | Ticket family in C-01; chain enabled (C-03, C-04); technician assigned with a number; role list resolves to 2 or 3 distinct people after dedupe (R3) and after skipping vacant rungs (R4) | Ringing rung 1 | Technician dialled (R2, G2). Chain length after dedupe recorded (MQ-4). |
...
| T4 | Ringing rung N | The ringing rung does not answer, is busy, or the dial fails | No further rung exists | Exhausted | Existing unconnected-call handling applies, unchanged (§1 Boundary). Ticket counted as not connected (M1, MQ-5). Terminal state. |
| T5 | Ringing rung N | Customer disconnects | — | Abandoned | Chain ends immediately; no further rung is dialled. Ticket counted as not connected (M1, MQ-5). Terminal state. |
| T6 | — | Customer-initiated call bridged, caller resolved to an in-scope ticket | No technician assigned to the ticket | Outside this spec | Today's routing runs unchanged; no chain is created (§1 Boundary, AC-REG-2). |
| T7 | — | Customer-initiated call bridged, caller resolved to an in-scope ticket | Technician, manager-tier user and owner resolve to one person after dedupe (R3) | Ringing rung 1 | That person dialled once (G3). No answer routes to T4, not T3. |
| T8 | — | Customer-initiated call bridged, caller resolved to an in-scope ticket | Role list cannot be resolved | Ringing rung 1 | **Failure envelope:** the call is never failed for want of a role list — it degrades to dialling the technician alone, which is today's behaviour. No answer routes to T4. How resolution is retried inside the call is the implementer's. ⚠️ *AI GENERATED — review* |
...
Internal screen. It must show the chain's outcomes, so §6's questions have an answer and M3 stays visible.
...
| Field — per-rung answer rate | computed (MQ-3) | feeds M3 and the independence check in §1 |
...
|---|---|---|---|---|
| C-01 | Ticket families the chain applies to (T1) | Service, Pickup | Any subset of the ticket families the IVR serves | Product |
| C-02 | Rung order and the role at each rung (R2, T1) | Rung 1 assigned technician · rung 2 manager-tier user · rung 3 owner | Fixed in V1 | Product |
...
|---|---|---|
| MQ-1 | Of in-scope tickets with at least one customer-initiated call, what share had at least one call where a person answered — split by ticket family. Abandoned chains count as not connected. | M1 · M2 |
| MQ-2 | For each chain, which rung answered — 1, 2, 3, or none — and whether that answer happened on the same inbound call the customer made. | G1 · M3 |
| MQ-3 | For each rung, the share of dials to that rung that were answered. | M3 · G2 · the independence check in §1 |
| MQ-4 | How many chains were shortened by dedupe, and to what length. | G3 · R3 |
| MQ-5 | How many chains ended Connected, Exhausted or Abandoned. | M1 definition · T4 · T5 |
| MQ-6 | Whether any person dialled in a chain did not belong to the ticket's CSP. | G4 invariant (R6) |
| MQ-7 | Whether any chain dialled a rung other than the assigned technician first, where a technician was assigned and had a number. | G2 invariant (R2) |
| MQ-8 | The time from bridge to answer or to customer disconnect, by the rung reached. ⚠️ *AI GENERATED — review* | Abandonment risk behind M1 · R7 |

---

## 7. Acceptance Criteria

**Example data used throughout** ⚠️ *AI GENERATED — review* — CSP `CSP-4412`: technician Ravi (09811100011), Manager Anil (09811100012), Owner Suresh (09811100013). Service ticket `TKT-88231` for customer Meena (09811100022). Calls on 12 Aug 2026.

### CHN — Chain creation and rung 1 (T1, T7, T8)

| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-CHN-1 | **Given** `TKT-88231` with Ravi assigned and Anil and Suresh on `CSP-4412`, **When** Meena calls at 15:20 and is resolved to that ticket, **Then** a chain of 3 rungs exists and Ravi's 09811100011 is the first and only number dialled. | R2 · T1 · G2 | Settled |
| AC-CHN-2 | **Given** `CSP-4412` where Suresh is both owner and the assigned technician on `TKT-88231`, and Anil is the manager, **When** Meena calls, **Then** the chain has 2 rungs — Suresh then Anil — and 09811100013 is dialled exactly once. | R3 · T1 · G3 | Settled |
...
| AC-CHN-4 | **Given** a CSP where the technician, manager and owner are all Suresh, **When** Meena calls, **Then** the chain has exactly 1 rung, 09811100013 is dialled once, and on no answer the chain ends Exhausted rather than advancing. | R3 · T7 · G3 | Settled |
| AC-CHN-5 | **Given** Ravi is assigned to `TKT-88231` but has no phone number on `CSP-4412`, **When** Meena calls, **Then** rung 1 is skipped without a dial and Anil's 09811100012 is dialled first. | R4 · T1 | Settled |
...
| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-WF-1 | **Given** `TKT-88231` with Ravi, Anil and Suresh, **When** Meena calls at 15:20, Ravi does not answer and Anil does, **Then** Meena has spoken to Anil on one call, was never asked to redial or press a key, and `TKT-88231` counts as connected at rung 2. | R1 · T1 · T3 · T2 · G1 | Settled |
...
| AC-REG-2 | **Given** a Service ticket at `CSP-4412` with no technician assigned, **When** Meena calls, **Then** routing is exactly as before this spec and no chain is created. | T6 · §1 Boundary | Settled |
...
| AC-REG-4 | **Given** an install ticket for Meena, **When** she calls, **Then** no chain is created and install routing is unchanged. | C-01 · §1 Boundary | Settled |
| AC-REG-5 | **Given** Anil answers as rung 2 on `TKT-88231`, **When** the call bridges, **Then** Anil receives no ticket reference, customer name or escalation indicator — unchanged from today. | §1 Boundary | Settled |
...
| AC-GRD-2 | **Given** 1,000 chains across the cohort, **When** the first dial of each is checked, **Then** every one dialled the assigned technician where a technician was assigned and had a number. | G2 · R2 · MQ-7 | Settled |
| AC-GRD-3 | **Given** any chain at a CSP where one person holds two or three of the roles, **When** the dial list is checked, **Then** no number appears twice. | G3 · R3 · MQ-4 | Settled |
...
| Rung | One position in an escalation chain, and the one person dialled at that position. Rung 1 is the assigned technician, rung 2 the manager-tier user, rung 3 the owner (C-02). | — |
...
| Abandoned | A chain where the customer disconnected before any rung answered. Counts as not connected (TL-9). | — |
...
|---|---|
| For a ticket, resolve the CSP's currently assigned technician, its manager-tier user and its owner, each with a dialable number, restricted to that CSP. | T1 · R2 · R4 · R6 · G4 |
| Reduce a resolved role list to distinct people before any dial, preserving rung order. | T1 · T7 · R3 · G3 |
...
| Fall back to dialling the technician alone when the role list cannot be resolved, without failing the call. | T8 |
...
|---|---|---|
| Header | Reviewer and all three Consulted names | Not supplied. Template requires named accountability. |
| §1 Boundary | Install tickets excluded from scope | Inferred: the problem was stated as restore and pickup, and install is the benchmark. Not explicitly ruled. |
...
| §3b T8 | Failure envelope — degrade to technician-only when the role list cannot be resolved | Default. PM did not state behaviour when the gateway-service lookup fails. |
